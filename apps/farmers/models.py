...
class FarmerProfile(models.Model):
    ...
    # Gender field for statistics
    gender = models.CharField(max_length=1, choices=GENDER_CHOICES, blank=True, null=True, help_text="For demographic reporting")
    ...
    def total_deliveries(self):
        ...
        return CoffeeBatch.objects.filter(farmer=self).count()
    
    def total_kgs_delivered(self):
        from apps.deliveries.models import CoffeeBatch
        from django.db.models import Sum
        result = CoffeeBatch.objects.filter(
            farmer=self,
            payment_status='paid'
